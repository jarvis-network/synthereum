...
createCliApp(
  buildCli(__filename).option('address', {
    type: 'string',
    coerce: address => {
      const returnAddr = assertIsAddress<42>(
        address || '0x6e30001f52C69948066Afd91B417a988c543d3F1',
      );
      return returnAddr;
    },
  }),
  async ({ web3, netId, argv: { address } }) => {
    ...
    logTable(
      events.map(
        ({
          ...
          to: formatAddr(to),
          value: `${formatAmount(
            wei(value).mul(wei(10 ** 12)) as Amount,
          )} USDC`,
          transactionHash,
        }),
      ),
