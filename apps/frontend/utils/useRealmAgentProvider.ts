...
import { PoolVersion } from '@jarvis-network/synthereum-contracts/dist/src/core/types/pools';
...
export function useRealmAgentProvider(
  ...
  function provide() {
    ...
    loadRealm(web3 as Web3On<SupportedNetworkName>, NETWORK_ID).then(realm => {
      realmAgent$.next(
        new RealmAgent(
          realm,
          address as AddressOn<typeof NETWORK_ID>,
          (process.env.NEXT_PUBLIC_POOL_VERSION || 'v1') as PoolVersion,
        ),
      );
    });
  }
