...
import { TASK_COMPILE, TASK_COMPILE_SOLIDITY_GET_COMPILER_INPUT } from "hardhat/builtin-tasks/task-names";
...
task("etherscan-verification", "Verifies contract on etherscan")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    if (hre.config.etherscan.apiKey === undefined || hre.config.etherscan.apiKey.trim() === "") {
    ...
    for (const { contractName, address } of deployed) {
      ...
      try {
        ...
        const response = await verifyContract(new URL (url), verificationRequest);
        console.log(
