...
contract('PerpetualLiquidatablePoolParty', function (accounts) {
  ...
  beforeEach(async () => {
    ...
    priceFeedIdentifier = web3.utils.padRight(
      web3.utils.utf8ToHex('TEST_IDENTIFIER'),
      64,
    );
    ...
    if (
      ...
      try {
        await LiquidatablePoolParty.link(feePayerPoolPartyLib);
        await LiquidatablePoolParty.link(perpetualPositionManagerPoolPartyLib);
        await LiquidatablePoolParty.link(perpetualLiquidatablePoolPartyLib);
      } catch (e) {
        // Allow this to fail in the Buidler case.
      }
    } else {
      // Truffle
      await LiquidatablePoolParty.link(
    ...
    liquidationContract = await LiquidatablePoolParty.new(constructorParams, {
      from: contractDeployer,
    });
    ...
    await collateralToken.addMinter(contractDeployer, {
      from: contractDeployer,
    });

    await collateralToken.mint(sponsor, amountOfCollateral, {
      from: contractDeployer,
    });
  ...
  describe('Creating a liquidation on a valid position', () => {
    ...
    it('Liquidator pays final fee', async () => {
      ...
      await collateralToken.mint(liquidator, finalFeeAmount, {
        from: contractDeployer,
      });
    ...
    it('Partial liquidation', async () => {
      ...
      await liquidationContract.requestWithdrawal(
        { rawValue: withdrawalAmount.toString() },
        { from: sponsor },
      );
  ...
  describe('Full liquidation has been created', () => {
    ...
    describe('Dispute a Liquidation', () => {
      ...
      it('Disputer does not have enough tokens', async () => {
        await collateralToken.transfer(contractDeployer, toWei('1'), {
          from: disputer,
        });
        assert(
          await didContractThrow(
      ...
      it('Throw if liquidation has already been disputed', async () => {
        ...
        await collateralToken.mint(disputer, finalFeeAmount, {
          from: contractDeployer,
        });
        ...
        await collateralToken.mint(disputer, disputeBond, {
          from: contractDeployer,
        });
        await collateralToken.increaseAllowance(
          liquidationContract.address,
  ...
  describe('Weird Edge cases', () => {
    ...
    it('Dispute rewards should not add to over 100% of TRV', async () => {
      ...
      assert(
        await didContractThrow(
          LiquidatablePoolParty.new(invalidConstructorParameter, {
            from: contractDeployer,
          }),
        ),
      );
    ...
    it('Dispute bond can be over 100%', async () => {
      ...
      const edgeLiquidationContract = await LiquidatablePoolParty.new(
      ...
      // Mint dispute bond to disputer
      await collateralToken.mint(disputer, edgeDisputeBond, {
        from: contractDeployer,
      });
      // Set allowance for contract to pull collateral tokens from sponsor
      await collateralToken.increaseAllowance(
  ...
  describe('Precision loss is handled as expected', () => {
    beforeEach(async () => {
      ...
      liquidationContract = await LiquidatablePoolParty.new(
        newTokenconstructorParams,
        { from: contractDeployer },
      );
      await syntheticToken.addMinter(liquidationContract.address);
