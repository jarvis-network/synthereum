...
contract('TIC', accounts => {
  ...
  it('should not mint tokens when there is insufficient collateral.', async () => {
    ...
    await expectRevert.unspecified(approveMintRequests(tic, accounts[1]));
    const newBalance = await syntheticToken.balanceOf(accounts[0]);
  ...
  it('should not be able to approve a mint request if it has been rejected.', async () => {
    ...
    await Promise.all(
      mintRequests.map(async mintRequest => {
  ...
  it('should let the LP recover collateral after contract expires.', async () => {
    ...
    const tokenValue = calculateTokenValue(syntheticBalance, price);
    const totalCollateral = tokenValue.add(
      web3.utils.toBN(web3.utils.toWei(lpCollateral)),
    );
  ...
  it('should let a user exchange one synthetic token for another', async () => {
    ...
    const blocktime = (await web3.eth.getBlock('latest'))['timestamp'];
    const otherDerivative = await createDerivative(
      networkId,
      finder.address,
      blocktime,
    );
    const otherTIC = await createTIC(
      otherDerivative.address,
      accounts[0],
      accounts[0],
    );
  ...
  it('should not be able to approve a exchange request if it has been rejected.', async () => {
    ...
    const exchangeRequests = await tic.getExchangeRequests({
      from: accounts[0],
    });
    await Promise.all(
      exchangeRequests.map(async exchangeRequest => {
        await tic.rejectExchange(exchangeRequest['exchangeID'], {
          from: accounts[1],
        });
        await expectRevert.unspecified(
          tic.approveExchange(exchangeRequest['exchangeID'], {
            from: accounts[1],
          }),
        );
      }),
  ...
  it('should not exchange tokens if a exchange request has already been approved.', async () => {
    ...
    await Promise.all(
      exchangeRequests.map(async exchangeRequest => {
        await tic.approveExchange(exchangeRequest['exchangeID'], {
          from: accounts[1],
        });
      }),
    );
  ...
  it('should remove a exchange request after it has been approved.', async () => {
    ...
    await approveExchangeRequests(tic, accounts[1]);
    const newExchangeRequests = await tic.getExchangeRequests({
      from: accounts[0],
    });
  ...
  it('should not be able to approve a redeem request if it has been rejected.', async () => {
    ...
    await Promise.all(
      redeemRequests.map(async redeemRequest => {
        await tic.rejectRedeem(redeemRequest['redeemID'], {
          from: accounts[1],
        });
        await expectRevert.unspecified(
  ...
  it('should not redeem tokens if a redeem request has already been approved.', async () => {
    ...
    await Promise.all(
      redeemRequests.map(async redeemRequest => {
        await tic.approveRedeem(redeemRequest['redeemID'], {
          from: accounts[1],
        });
      }),
    );
  ...
  it('should remove a redeem request after it has been approved.', async () => {
    ...
    await approveRedeemRequests(tic, accounts[1]);
    const newRedeemRequests = await tic.getRedeemRequests({
      from: accounts[0],
    });
