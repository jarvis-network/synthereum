...
module.exports = {
  ...
  createFinder: async (networkId, mockOracleAddress) => {
    ...
    const identifierWhitelistInterfaceName = web3Utils.toHex(
      'IdentifierWhitelist',
    );
    await finder.changeImplementationAddress(
  ...
  createMintRequest: async (
    ...
    const totalCollateral = web3Utils
      .toBN(userCollateral)
      .add(mintFee)
      .toString();
  ...
  createRedeemRequest: async (
    ...
    const userCollateral = web3Utils.toWei(collateralAmount);
    await syntheticToken.approve(tic.address, userCollateral, {
      from: account,
    });
  ...
  createExchangeRequest: async (
    ...
    await tic.exchangeRequest(destTIC.address, numTokensWei, destNumTokensWei, {
      from: account,
    });
  },
  ...
  approveExchangeRequests: async (tic, account) => {
    ...
    await Promise.all(
      exchangeRequests.map(exchangeRequest => {
        return tic.approveExchange(exchangeRequest['exchangeID'], {
          from: account,
        });
      }),
    );
  ...
  rejectExchangeRequests: async (tic, account) => {
    ...
    await Promise.all(
      exchangeRequests.map(exchangeRequest => {
        return tic.rejectExchange(exchangeRequest['exchangeID'], {
          from: account,
        });
      }),
    );
  ...
  settleExpired: async (tic, syntheticToken, account, syntheticBalance) => {
    await syntheticToken.approve(tic.address, syntheticBalance, {
      from: account,
    });
