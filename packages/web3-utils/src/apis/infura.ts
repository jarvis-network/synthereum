...
import { Network, NetworkName, toNetworkName } from '../eth/networks';
...
export function getInfuraEndpoint<Net extends Network>(
  ...
  return `${protocol}//${networkName}.infura.io/v3/${projectId}` as Tagged<string, Net>;
}

export function getInfuraWeb3<Net extends NetworkName>(
  network: Net,
  protocol: Protocol = Protocol.https,
): Tagged<Web3, Net> {
  ...
  return result as Tagged<Web3, Net>;
}
